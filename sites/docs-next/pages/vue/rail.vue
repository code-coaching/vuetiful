...
const scriptExample =
  "import { VRail, VRailTile } from '@code-coaching/vuetiful';";
...
<template>
  ...
  <section class="section">
    <v-code-block
      language="ts"
      :code="`import { VRail, VRailTile, useRail } from '@code-coaching/vuetiful';`"
    />
  </section>
  ...
  <section class="section">
    <v-alert type="info">
      <div>
        <v-badge>VRail</v-badge> is a container that holds multiple
        <v-badge>VRailTile</v-badge> components.
      </div>
      <div>
        <v-badge>VRailTile</v-badge> is a clickable tile that indicates the
        current page and allows navigation through a click handler. Each tile
        has a unique value, determining its active state, which is automatically
        stored in the <v-badge>selectedRailTile</v-badge> variable accessible
        via <v-badge>useRail</v-badge>.
      </div>
    </v-alert>
  </section>
      ...
      <v-tab-panel>
        ...
        <section class="section">
          <v-preview>
            <template v-slot:preview>
              <div
                class="mb-4 flex h-[576px] w-full flex-col items-center justify-center gap-2 md:h-[576px]"
              >
                <v-shell class="flex border-2 border-surface-500 min-h-full">
                  <div
                    :class="'sidebar grid h-full grid-cols-[auto_1fr] border-r border-surface-500/30'"
                  >
                    <v-rail class="border-r border-surface-500">
                      <v-rail-tile value="demo-1" label="Vuetiful" />
                      <v-rail-tile value="demo-2" label="Is" />
