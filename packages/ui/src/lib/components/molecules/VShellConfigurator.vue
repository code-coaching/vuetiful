...
defineProps({
  ...
  slotFixedHeader: { type: String as () => CssClasses, default: 'z-0' },
...
<template>
  <div class="flex flex-col w-full h-full overflow-hidden vuetiful-shell">
    <header v-if="$slots.fixedHeader" :class="`vuetiful-fixed-header ${slotFixedHeader}`">
      <slot name="fixedHeader" />
    </header>

    <div class="flex flex-auto w-full h-full overflow-hidden vuetiful-shell-content">
      <aside
